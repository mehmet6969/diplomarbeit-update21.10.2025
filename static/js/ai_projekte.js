...
function saveSession(email) {
    ...
    localStorage.setItem('ai_auth_session', JSON.stringify(session));
}

function loadSession() {
    try {
        const sessionData = localStorage.getItem('ai_auth_session');
        if (!sessionData) return null;

        const session = JSON.parse(sessionData);

        if (Date.now() > session.expiresAt) {
            localStorage.removeItem('ai_auth_session');
            return null;
        }
    ...
    useEffect(() => {
        ...
        try {
            ...
            for (let i = 0; i < nodes.length - 5; i++) {
                for (let j = 0; j < 3; j++) {
                    const points = [nodes[i].position, nodes[Math.min(i + 3 + j, nodes.length - 1)].position];
            ...
            const animate = () => {
                ...
                nodes.forEach((node, i) => {
                    node.material.opacity = 0.5 + Math.sin(Date.now() * 0.002 + i) * 0.3;
                });
...
function ImportModal({ isOpen, onClose, onSave }) {
    ...
    const categories = ['Quantum AI', 'Computer Vision', 'NLP', 'Robotics AI', 'Generative AI', 'Medical AI', 'Predictive Analytics'];
    ...
    const handleSubmit = () => {
        ...
        setFormData({
            title: '', description: '', category: 'NLP', badge: '',
            complexity: 4, year: new Date().getFullYear().toString(), duration: '',
            ...
            <div className="modal-content import-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    <div className="import-header">
                        <h2>Neues AI-Projekt importieren</h2>
                    ...
                    <div className="form-grid">
                        ...
                        <div className="form-group full-width">
                            ...
                            <input type="text" value={formData.title} onChange={(e) => handleChange('title', e.target.value)} placeholder="z.B. Neural Network Analyzer" />
                        </div>

                        <div className="form-group">
                            <label>Badge</label>
                            <input type="text" value={formData.badge} onChange={(e) => handleChange('badge', e.target.value)} placeholder="z.B. Research, Production" />
                        </div>
                        ...
                        <div className="form-group">
                            <label>Dauer</label>
                            <input type="text" value={formData.duration} onChange={(e) => handleChange('duration', e.target.value)} placeholder="z.B. 6 Monate" />
                        </div>
                        ...
                        <div className="form-group full-width">
                            ...
                            <TagsInput tags={formData.tags} setTags={(tags) => handleChange('tags', tags)} placeholder="z.B. Deep Learning, NLP, CNN..." />
                        </div>

                        <div className="form-group full-width">
                            <label>Technologien (Enter zum Hinzufügen)</label>
                            <TagsInput tags={formData.technologies} setTags={(tech) => handleChange('technologies', tech)} placeholder="z.B. PyTorch, TensorFlow, Python..." />
                        </div>
...
function App() {
    ...
    const toggleVisibility = async (project) => {
        try {
            ...
            console.error("Sperrvermerk-Update fehlgeschlagen:", e);
        ...
        <React.Fragment>
            <nav id="navbar">
                <div className="nav-content">
                    <div className="logo">🧠 AI Lab</div>
                    <ul className="nav-links">
                        <li><a href="/" className="nav-link">Home</a></li>
                        <li><a href="/artificial_intelligence_projekte" className="nav-link active">Projekte</a></li>
                        <li><a href="/artificial_intelligence_info" className="nav-link">Info</a></li>
                        <li>
            ...
            <section className="hero">
                ...
                <div className="hero-content">
                    ...
                    <p className="hero-subtitle">
                        Innovative KI-Projekte von der Forschung bis zur Anwendung
                    </p>
                            ...
                            <div key={project.id} className="project-card" onClick={() => setSelectedProject(project)}>
                                <div className="project-image" style={{ backgroundImage: `url(${project.image})` }}>
                                    ...
                                    {project.visibility === "restricted"
                                        ? <div className="project-badge">🔒 Gesperrt</div>
                                ...
                                <div className="project-content">
                                    ...
                                    <div className="project-meta">
                                        <div className="meta-item"><span>📅</span><span>{project.year}</span></div>
                                        <div className="meta-item"><span>⏱</span><span>{project.duration}</span></div>
                                        <div className="meta-item"><span>👥</span><span>{project.team}</span></div>
                                    </div>
                                    <div className="project-tags">
                                        {project.tags && project.tags.map((tag, i) => <span key={i} className="project-tag">{tag}</span>)}
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            <div className="modal-header">
                                ...
                                <div className="project-tags">
                                    {selectedProject.tags && selectedProject.tags.map((tag, i) => <span key={i} className="project-tag">{tag}</span>)}
                                </div>
                                ...
                                {isAuthenticated && (
                                    <div style={{ display: "flex", gap: "1rem", marginTop: "1.5rem", flexWrap: "wrap" }}>
                                        ...
                                        <button
                                            ...
                                            onClick={() => toggleVisibility(selectedProject)}
                                        >
                            ...
                            {selectedProject.features && selectedProject.features.filter(f => f).length > 0 && (
                                <div className="modal-section">
                            ...
                            {selectedProject.challenges && (
                                <div className="modal-section">
                                    ...
                                    <p style={{ color: 'var(--silver)', lineHeight: '1.8', paddingLeft: '2rem' }}>{selectedProject.challenges}</p>
                                </div>
                            )}
                            ...
                            {selectedProject.outcome && (
                                <div className="modal-section">
                                    <h3>Ergebnis</h3>
                                    <p style={{ color: 'var(--silver)', lineHeight: '1.8', paddingLeft: '2rem' }}>{selectedProject.outcome}</p>
                                </div>
                            )}
            ...
            <footer>
                ...
                <div className="footer-bottom">
                    <p>© 2025 IEM HTL Dornbirn. AI Excellence.</p>
                </div>
            </footer>
        </React.Fragment>
    );
